...
- **顶部**：搜索框（实时搜 title/owner/remark）、主题切换、导入 JSON、导出 JSON、新建顶层事项
- **左侧看板**：总数、完成率（环形进度）、按状态/优先级统计
- **筛选区**：状态、优先级、负责人多维筛选，4 视图共享筛选
- **主区**：5 个视图 Tab 切换
  - **列表**：树形展开，点击 chevron 收起，行内编辑按钮
  - **看板**：按 status 分列卡片墙，拖卡片改状态
  - **甘特**：按 plan_end 排期的水平时间条；任务名过长自动截断显示 `...`，鼠标悬停任务名或时间条弹出**自定义浮动 tooltip** 展示完整标题（无原生 title 干扰）
  - **日历**：月历视图，按状态色码区分事件；每个日期格最多显示 3 条，超出部分隐藏；**点击日期格空白处弹出当日事项弹窗**，弹窗内列出当天全部事项（含状态色条、标题、负责人、优先级、进度），支持滚动；点击弹窗内具体事项直接打开编辑视图
  - **归档**：独立 Tab，只读列表，搜索/状态筛选，批量恢复或永久删除
- **事项详情面板**：标题、负责人、状态、优先级、进度、计划/实际完成时间、备注、标签、依赖、评论；其中**依赖事项选择使用自定义多选组件**（非原生 `<select multiple>`），点击条目切换选中态（蓝底 + 勾选标记），顶部展示已选标签可单独移除；自动排除自身及后代节点防止循环依赖
...
顶部"导出"按钮调 `GET /api/export` 拉全量 JSON 触发浏览器下载，文件名格式 `todolist_export_YYYYMMDD.json`。

- **导出范围**：包含**未归档 + 已归档**全部事项及评论，作为完整数据快照
- **用途**：跨工具迁移、外部备份、数据快照存档

#### 导入（数据恢复 / 合并）

顶部"导入"按钮（位于"导出"按钮旁）选择本地 JSON 文件后执行：

1. 前端读取文件并解析 JSON，弹窗显示待导入事项条数，确认后调 `POST /api/import`
2. 后端按 `parent_id` 做**拓扑排序**，保证父事项先于子事项插入
3. **ID 冲突跳过**：数据库中已存在的 id 自动跳过，不覆盖现有数据
4. **单事务保证**：导入过程包在一个事务内，任一条目失败则全部回滚
5. **评论一并导入**：事项关联的评论随事项一起写入
6. 前端 toast 反馈成功 / 跳过 / 失败条数，文件输入框重置以便重复选择同一文件

> 提示：导入的 JSON 格式与导出文件一致，可直接用导出文件作为导入源。导入不会删除或覆盖已有数据，可安全重试。
...
- **归档 Tab**：点顶部"📦 归档"查看所有已归档事项
  - 搜索 / 按状态筛选
  - 勾选多条 → "恢复选中"批量回主视图，或点红色"批量删除"按钮一次性永久删除（请求体 `{ids: [...]}`，CASCADE 级联删子项）
